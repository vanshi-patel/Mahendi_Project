...
<head>
    ...
    <title>Manage Western Images - Mahendi Magic Hub</title>
...
<body class="light-mode">
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
        ...
        if ($action === 'add') {
            if (isset($_FILES['image_file'])) {
                ...
                $uploadDir = 'images/western/'; // Folder where images will be uploaded
                ...
                if (move_uploaded_file($fileTmp, $filePath)) {
                    $stmt = $conn->prepare("INSERT INTO western_images(image_name, image_path, price) VALUES (?, ?, ?)");
        ...
        if ($action === 'edit' && isset($_POST['id'])) {
            ...
            if (isset($_FILES['image_file']) && $_FILES['image_file']['error'] == 0) {
                ...
                $fileName = $file['name'];
                $fileTmp = $file['tmp_name'];
                $uploadDir = 'images/western/';

                $filePath = $uploadDir . basename($fileName);

                // Move new file and update path in database
                if (move_uploaded_file($fileTmp, $filePath)) {
                    $stmt = $conn->prepare("UPDATE western_images SET image_name = ?, image_path = ?, price = ? WHERE id = ?");
                ...
                $stmt = $conn->prepare("UPDATE western_images SET image_name = ?, price = ? WHERE id = ?");
        ...
        if ($action === 'delete' && isset($_POST['id'])) {
            ...
            $stmt = $conn->prepare("SELECT image_path FROM western_images WHERE id = ?");
            ...
            $stmt = $conn->prepare("DELETE FROM western_images WHERE id = ?");
            $stmt->bind_param("i", $id);
            $stmt->execute();
            $stmt->close();
    ...
    <div class="admin-header">
        <h1>Manage Western Images</h1>
    </div>

    <div class="admin-content">
        <section class="card">
            <h2>Western Images</h2>
            ...
            <!-- Table for managing Western images -->
            <?php
            // Fetch Western images
            $query = "SELECT * FROM western_images";
