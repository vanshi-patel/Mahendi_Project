...
<head>
    ...
    <title>Manage Baby-Shower Images - Mahendi Magic Hub</title>
...
<body class="light-mode">
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
        ...
        if ($action === 'add') {
            if (isset($_FILES['image_file'])) {
                ...
                $uploadDir = 'images/baby-shower/'; // Folder where images will be uploaded
                ...
                if (move_uploaded_file($fileTmp, $filePath)) {
                    $stmt = $conn->prepare("INSERT INTO babyshower_images(image_name, image_path, price) VALUES (?, ?, ?)");
        ...
        if ($action === 'edit' && isset($_POST['id'])) {
            ...
            if (isset($_FILES['image_file']) && $_FILES['image_file']['error'] == 0) {
                ...
                $fileName = $file['name'];
                $fileTmp = $file['tmp_name'];
                $uploadDir = 'images/baby-shower/';

                $filePath = $uploadDir . basename($fileName);

                // Move new file and update path in database
                if (move_uploaded_file($fileTmp, $filePath)) {
                    $stmt = $conn->prepare("UPDATE babyshower_images SET image_name = ?, image_path = ?, price = ? WHERE id = ?");
                ...
                $stmt = $conn->prepare("UPDATE babyshower_images SET image_name = ?, price = ? WHERE id = ?");
        ...
        if ($action === 'delete' && isset($_POST['id'])) {
            ...
            $stmt = $conn->prepare("SELECT image_path FROM babyshower_images WHERE id = ?");
            ...
            $stmt = $conn->prepare("DELETE FROM babyshower_images WHERE id = ?");
            $stmt->bind_param("i", $id);
            $stmt->execute();
            $stmt->close();
    ...
    <div class="admin-header">
        <h1>Manage Baby-Shower Images</h1>
    </div>

    <div class="admin-content">
        <section class="card">
            <h2>Baby-Shower Images</h2>
            ...
            <!-- Table for managing Baby-Shower images -->
            <?php
            // Fetch Baby-Shower images
            $query = "SELECT * FROM babyshower_images";
